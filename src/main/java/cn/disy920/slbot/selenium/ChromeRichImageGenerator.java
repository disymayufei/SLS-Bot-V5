...
public class ChromeRichImageGenerator implements RichImageGenerator {

    private boolean hadInit = false;

    private final File chromeBinaryFile = new File(PLUGIN_INSTANCE.getDataFolder(), "Selenium/Chrome/Binary/google-chrome");
    private final File chromeDriverFile = new File(PLUGIN_INSTANCE.getDataFolder(), "Selenium/Chrome/Driver/chromedriver");
    private final File queryServerHtmlFile = new File(PLUGIN_INSTANCE.getDataFolder(), "Selenium/HTML/query-server.html");

    private ChromeDriver driver;

    public ChromeRichImageGenerator() {
        new File(PLUGIN_INSTANCE.getDataFolder(), "Selenium/Chrome").mkdirs();
    ...
    @Override
    public void init() {
        if (!chromeBinaryFile.exists() || chromeBinaryFile.isDirectory()){
            RECORDER.error("未检测到Chrome浏览器二进制文件，请完整下载适用于您系统的Chrome浏览器后，将其置于目录" + chromeBinaryFile.getAbsolutePath() + "下");
            hadInit = false;
            return;
        }

        if (!chromeDriverFile.exists() || chromeDriverFile.isDirectory()){
            RECORDER.error("未检测到Chrome驱动，请完整下载适用于Selenium的Chrome驱动，将其置于目录" + chromeDriverFile.getAbsolutePath() + "下");
            hadInit = false;
            return;
        }
    ...
    public InputStream genQueryServerImage(int currentNum, int maxNum, int imageHeight, @NotNull String text, int theme) {
        if (!hadInit){
            LOGGER.warn("Chrome尚未初始化完成，请检查一下或稍后再试");
            return null;
        }
        ...
        try {
            ...
            // 渲染并截图
            return driver.getScreenshotAs(ImageOutputType.INPUT_STREAM);
        }
        catch (Exception e){
